...
while c != 'q' and c != 'Q':
    ...
    g = [[0 for c in range(GRID_X)] for r in range(GRID_Y)]

    for robot in robots:
        f = tuple(map(lambda a, b, c: (a + (counter * b))%c, robot[0], robot[1], (GRID_X, GRID_Y)))
        g[f[1]][f[0]] += 1
    g = [['X' if p > 0 else '.' for p in r ]for r in g]
    g = [''.join(r) for r in g]

    stop = False
    for r in g:
        if "XXXXXXXXXXXXXXXXX" in r:
            stop = True
    if stop:
        print(*g, sep='\n')
        print(f"Counter is: {counter}, continue?")
        c = input()
